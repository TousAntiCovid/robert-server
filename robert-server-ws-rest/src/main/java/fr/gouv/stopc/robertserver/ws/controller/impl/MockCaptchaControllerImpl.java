...
public class MockCaptchaControllerImpl implements ICaptchaController {
    ...
    private ResponseEntity<byte[]> getCaptchaCommon(String captchaId, String mediaType) {
         ...
         * int width = 1; int height = 2; DataBuffer buffer = new
         * DataBufferByte(aByteArray, aByteArray.length); WritableRaster raster =
         * Raster.createInterleavedRaster(buffer, width, height, 3 * width, 3, new int[]
         * { 0, 1, 2 }, (Point) null); ColorModel cm = new
         * ComponentColorModel(ColorModel.getRGBdefault().getColorSpace(), false, true,
         * Transparency.OPAQUE, DataBuffer.TYPE_BYTE); BufferedImage image = new
