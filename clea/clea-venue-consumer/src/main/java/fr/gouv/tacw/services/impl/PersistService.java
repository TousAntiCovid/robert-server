...
public class PersistService implements IPersistService {
    ...
    public void checkAndPersist(DetectedVenue detectedVenue) {
        ...
        } catch (CleaEncryptionException e1) {
            log.error("");  // FIXME
        }
    }
    ...
    private Optional<DetectedVenue> verify(DetectedVenue detectedVenue) throws CleaEncryptionException {
    ...
    private boolean isComputedTemporaryLocationPublicIdValid(DetectedVenue detectedVenue) throws CleaEncryptionException {
        UUID computed = new CleaEciesEncoder().computeLocationTemporaryPublicId(detectedVenue.getLocationTemporarySecretKey());
    ...
    private boolean isFresh(DetectedVenue detectedVenue) {
        ...
        if (qrCodeRenewalInterval <= 0)
            return true;
        return Math.abs(TimeUtils.ntpTimestampFromInstant(detectedVenue.getQrCodeScanTime()) - detectedVenue.getQrCodeValidityStartTime()) < (qrCodeRenewalInterval + 300 + 300);
    }
