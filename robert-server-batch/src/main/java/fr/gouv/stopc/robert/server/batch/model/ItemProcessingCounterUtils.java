...
/**
 * Utility class used by Registration or Contact Id Mapping processors to get
 * the mapped id. The last mapped id gives the number of mapped items and is
 * used by the partitioner to split entries among the available workers. Workers
 * will process in parallel. Since the id mapping is run sequentially, no need
 * to synchronize incrementCurrentIdOfItemIdMapping.
 */
@Getter
