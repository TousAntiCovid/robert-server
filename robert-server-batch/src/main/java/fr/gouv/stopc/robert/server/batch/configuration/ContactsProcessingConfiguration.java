...
public class ContactsProcessingConfiguration {
    ...
    @Bean
    public MongoItemReader<Registration> registrationIdMappingForScoredRegistrationReader(MongoTemplate mongoTemplate) {

        return mongoRegistrationIdMappingReader(mongoTemplate, "{exposedEpochs: {$ne: []}}");
    }

    @Bean
    public Step populateIdMappingWithScoredRegistrationStep(
            MongoItemReader<Registration> registrationIdMappingForScoredRegistrationReader,
            MongoItemWriter<ItemIdMapping> mongoRegistrationIdMappingItemWriter) {

        return this.stepBuilderFactory.get("populateIdMappingWithScoredRegistrationStep")
                .<Registration, ItemIdMapping>chunk(POPULATE_STEP_CHUNK_SIZE)
                .reader(registrationIdMappingForScoredRegistrationReader)
                .processor(registrationIdMappingProcessor())
                .writer(mongoRegistrationIdMappingItemWriter).build();
    }
    ...
    public Step processRegistrationRiskWorkerStep(MongoItemReader<Registration> mongoRegistrationItemReader,
            ItemProcessor<Registration, Registration> registrationRiskProcessor,
            MongoItemWriter<Registration> registrationItemWriter) {
        ...
        return this.stepBuilderFactory.get("processRegistrationRiskWorkerStep")
                .<Registration, Registration>chunk(CHUNK_SIZE)
                .reader(mongoRegistrationItemReader)
                .processor(registrationRiskProcessor)
                .writer(registrationItemWriter)
                .build();
    }
    ...
    public Step processRegistrationRiskStep(Step processRegistrationRiskWorkerStep) {
        ...
        return this.stepBuilderFactory.get("processRegistrationRiskStep")
                .partitioner("processRegistrationRiskPartitioner", partitioner())
                .partitionHandler(partitionHandler(processRegistrationRiskWorkerStep, this.asyncTaskExecutor()))
                .build();
    }
