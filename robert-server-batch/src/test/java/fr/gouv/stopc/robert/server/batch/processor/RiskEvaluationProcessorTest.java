...
public class RiskEvaluationProcessorTest {
    ...
    private final static long SERVICE_TIME_START_TIMESTAMP = SERVICE_TIME_START.getEpochSecond()
            + TimeUtils.SECONDS_FROM_01_01_1900_TO_01_01_1970;
    ...
    public void beforeEach() {
        ...
        lastExpositionEpoch = arbitraryScoreEpochStart + TimeUtils.EPOCHS_PER_DAY * 9;
        lastContactDate = TimeUtils
                .convertUnixMillistoNtpSeconds(Instant.now().minus(6, ChronoUnit.DAYS).toEpochMilli());
        expectedLastContactDate = TimeUtils
                .dayTruncatedTimestamp(TimeUtils.getNtpSeconds(this.lastExpositionEpoch, SERVICE_TIME_START_TIMESTAMP));
    ...
    public void testWhenScoresAtRiskThenRiskDetected() {
        ...
        this.registration.get().setExposedEpochs(expositions);
        this.registration.get().setLastContactTimestamp(lastContactDate);
    ...
    public void testWhenManyExpositionsGivingScoreAtRiskThenGetLastAtRiskExpositionDate() {
        ...
        assertThat(returnedRegistration.getLatestRiskEpoch()).isEqualTo(this.currentEpoch);
        log.info(
                "Service time start: {}, Arbitrary score epoch start: {}",
                SERVICE_TIME_START,
