...
@ContextConfiguration(classes = { ScoringProperties.class })
public class ExposedTokenGeneratorTest {
	...
	public void testNumberOfGeneratedTokenIsOk() {
		...
		List<ExposedStaticVisitEntity> tokens = new ExposedTokenGenerator(visit, scoringProperties).generateAllExposedTokens().collect(Collectors.toList());
	...
	public void testGeneratedTokensIncludesDerivedToken() {
		...
		Stream<ExposedStaticVisitEntity> exposedVisits = new ExposedTokenGenerator(visit, scoringProperties).generateAllExposedTokens();
