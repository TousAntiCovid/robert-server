package fr.gouv.tousantic.robert.analytics.server.config.security;

import fr.gouv.tousantic.robert.analytics.server.config.security.oauth2tokenvalidator.DelegatingOAuth2JwtTokenValidator;
import fr.gouv.tousantic.robert.analytics.server.config.security.oauth2tokenvalidator.ExpPresenceOAuth2TokenValidator;
import fr.gouv.tousantic.robert.analytics.server.config.security.oauth2tokenvalidator.JtiCanOnlyBeUsedOnceOAuth2TokenValidator;
import fr.gouv.tousantic.robert.analytics.server.config.security.oauth2tokenvalidator.JtiPresenceOAuth2TokenValidator;
...
public class WebSecurityConfiguration extends WebSecurityConfigurerAdapter {
    ...
    public JwtDecoder jwtDecoder(final JtiPresenceOAuth2TokenValidator jtiPresenceOAuth2TokenValidator,
                                 ...
                                 final ExpPresenceOAuth2TokenValidator expPresenceOAuth2TokenValidator) {
        final NimbusJwtDecoder jwtDecoder = NimbusJwtDecoder.withPublicKey(robertRSAPublicKey).build();
        final DelegatingOAuth2JwtTokenValidator delegatingTokenValidator = new DelegatingOAuth2JwtTokenValidator(expPresenceOAuth2TokenValidator, new JwtTimestampValidator(), jtiPresenceOAuth2TokenValidator, jtiCanOnlyBeUsedOnceOAuth2TokenValidator);
