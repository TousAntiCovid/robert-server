package fr.gouv.tousantic.robert.analytics.server.it;

import com.fasterxml.jackson.databind.ObjectMapper;
import fr.gouv.tousantic.robert.analytics.server.RobertAnalyticsServerApplication;
import fr.gouv.tousantic.robert.analytics.server.controller.vo.AnalyticsVo;
import fr.gouv.tousantic.robert.analytics.server.controller.vo.ErrorVo;
import fr.gouv.tousantic.robert.analytics.server.model.kafka.Analytics;
...
@SpringBootTest(classes = RobertAnalyticsServerApplication.class)
...
public class AnalyticsCreationOauth2ErrorTest {
    ...
    public void itShouldRejectTokenWithoutJTI() throws Exception {
        ...
        final ErrorVo errorVo = objectMapper.readValue(mvcResult.getResponse().getContentAsString(), ErrorVo.class);
        assertThat(errorVo.getMessage()).isEqualTo("An error occurred while attempting to decode the Jwt: The token identifier (jti) is missing");
        assertThat(errorVo.getTimestamp()).isEqualToIgnoringSeconds(ZonedDateTime.now());
    ...
    public void itShouldRejectTokenWithAnAlreadyUsedJTI() throws Exception {
        ...
        final ErrorVo errorVo = objectMapper.readValue(mvcResult.getResponse().getContentAsString(), ErrorVo.class);
        assertThat(errorVo.getMessage()).isEqualTo("An error occurred while attempting to decode the Jwt: The token identifier (jti) can only be used once");
        assertThat(errorVo.getTimestamp()).isEqualToIgnoringSeconds(ZonedDateTime.now());
    ...
    @Test
    public void itShouldRejectTokenWithoutExp() throws Exception {
        final AnalyticsVo analyticsVo = buildAnalyticsVo();
        final String analyticsAsJson = objectMapper.writeValueAsString(analyticsVo);
        ...
        final ErrorVo errorVo = objectMapper.readValue(mvcResult.getResponse().getContentAsString(), ErrorVo.class);
        assertThat(errorVo.getMessage()).startsWith("An error occurred while attempting to decode the Jwt: The token expiration (exp) is missing");
        assertThat(errorVo.getTimestamp()).isEqualToIgnoringSeconds(ZonedDateTime.now());
