...
/**
 * Service centralizing crypto operations required to generate or validate
 * crypto tokens
 */
public interface CryptoService {

    /**
     * Generating Tuple ECC EBID and epoch performing encryption with cryptoForEBID
     * and cryptoForECC EBID ->
     * {@link #generateEBID(CryptoCipherStructureAbstract, int, byte[])} ECC ->
     * {@link #encryptCountryCode(CryptoCipherStructureAbstract, byte[], byte)}
     *
    ...
    /**
     * @param cryptoForEBID instance of Crypto algo to encrypt EBID with, using KS
     *                      (server key)
     ...
     * @throws RobertServerCryptoException
     */
    byte[] generateEBID(CryptoCipherStructureAbstract cryptoForEBID, int epochId, byte[] idA)
            throws RobertServerCryptoException;
    ...
    /**
     * @param cryptoForECC instance of Crypto algo to encrypt ECC with, using KG
     *                     (federation key)
     * @param ebid         Result of encryption of IDa and i as EBID - 64-bits
     * @param countryCode  country code - 8-bits
     * @return return encrypted countryCode and EBID as ECC with MSB method
     *         specified in ROBert documentation.
     * @throws RobertServerCryptoException
     */
    byte[] encryptCountryCode(CryptoCipherStructureAbstract cryptoForECC, byte[] ebid, byte countryCode)
            throws RobertServerCryptoException;
     ...
     * @throws RobertServerCryptoException
     */
    byte[] decryptCountryCode(CryptoCipherStructureAbstract cryptoForECC, byte[] ebid, byte encryptedCountryCode)
            throws RobertServerCryptoException;
    ...
    /**
     * @param cryptoHMACSHA256S CryptoStructure used to encrypt HMAC-SHA256
     * @param toBeEncrypted     is concat(EBID | Time) respectively 32-bits +
     *                          64-bits
     ...
     * @param toBeEncrypted     Payload to be encrypted (with the additional
     *                          prefixByte parameter)
     * @param macToVerify       The MAC to be verified
     * @param salt              The type of the request (0x02=ESR,
     *                          0x03=Unregister...)
